            ...
            <SheetContent className="overflow-y-hidden flex flex-col gap-0">
                ...
                <section className="mt-4 flex-1 overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            {order.Account?.Personal?.nationality && (
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm">{order.Account.Personal.nationality}</p>
                                </div>
                            )}
                            {order.channel && (
                                <div className="space-y-1">
                                    <span className="text-sm text-muted-foreground">Canal</span>
                                    <p className="text-sm uppercase">{order.channel === "base" ? "registrodeimei.cl" : order.channel}</p>
                                </div>
                            )}
                            {order.purchase_number && (
                                <div className="space-y-1">
                                    <span className="text-sm text-muted-foreground">Número de Compra</span>
                                    <p className="text-sm">{order.purchase_number}</p>
                                </div>
                            )}
                            {order.registrant_name && (
                                <div className="space-y-1">
                                    <span className="text-sm text-muted-foreground">Registrante</span>
                            ...
                            <Carousel className="mx-auto select-none">
                                <CarouselContent>
                                    {order.Imei.map((imei, index) => (
                                        <CarouselItem key={imei.imei_number} className="cursor-grab">
                                            <div className="p-4 border border-border rounded-md space-y-2">
                                                ...
                                                <div className="grid grid-cols-2 gap-1 text-sm">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="font-mono">{imei.imei_number}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                    ...
                    <div className="py-4 px-6 space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-4 border border-border rounded-lg p-4">
                            <div className="flex flex-col items-center gap-2 text-sm">
                                <Box className={`h-5 w-5 ${order.has_registration ? 'text-green-500' : 'text-destructive'}`} />
                                <span className="flex items-center gap-1">Registro IMEI
                            ...
                            <div className="flex flex-col items-center gap-2 text-sm">
                                <ShieldCheck className={`h-5 w-5 ${order.has_antivirus ? 'text-green-500' : 'text-destructive'}`} />
                                <span className="flex items-center gap-1">Antivirus
                                    {order.has_antivirus ? (
                                        <Check className="h-4 w-4 text-green-500" />
                                    ) : (
                                        <X className="h-4 w-4 text-destructive" />
                            ...
                            <div className="flex flex-col items-center gap-2 text-sm">
                                <FileText className={`h-5 w-5 ${order.has_insurance ? 'text-green-500' : 'text-destructive'}`} />
                                <span className="flex items-center gap-1">Seguro
                                    {order.has_insurance ? (
                                        <Check className="h-4 w-4 text-green-500" />
                                    ) : (
                                        <X className="h-4 w-4 text-destructive" />
                    ...
                    {(order.imei_excel_url ||
                        ...
                        (order.Account?.Personal?.id_card_url && !order.Account.is_business)) && (
                            <div className="py-4 px-6 space-y-2">
